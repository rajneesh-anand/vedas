...
const CourseVideoTwo = () => {
  const router = useRouter();
  const studentClass = router.query.class;
  ...
  useEffect(async () => {
    ...
    if (studentClass) {
      const classFilter = ClassOptions.find(
        (item) => item.value === studentClass
      );
      if (classFilter) {
        setSelectedClass(classFilter);
        let searchText = `${classFilter.name}-${selectedMedium}`;
  ...
  }, [studentClass, selectedClass, selectedMedium]);
