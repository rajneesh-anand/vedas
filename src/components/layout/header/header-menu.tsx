...
const HeaderMenu: React.FC<MenuProps> = ({ data, className }) => {
  ...
  return (
    <nav className={cn('headerMenu flex relative -mx-3 xl:-mx-4', className)}>
      {data?.map((item: any) => (
        <div
        ...
        >
          <Link
            href={item.path}
            className="inline-flex items-center text-sm lg:text-15px text-red-700 py-2 font-normal relative group-hover:text-skin-primary"
          >
            {t(item.label)}
